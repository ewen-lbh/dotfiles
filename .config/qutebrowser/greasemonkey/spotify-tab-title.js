// ==UserScript==
// @name Spotify Title for Polybar -- Userscript
// @namespace https://open.spotify.com
// @description Replaces the tab's title with the a JSON string of metadata about the current playback
...
function updateState(mutations) {
	...
	// I suspect it even does anything performance-wise.
	if (JSON.stringify(state) == JSON.stringify(previousState)) {
		return
	}
	let request = new XMLHttpRequest();
	request.open("GET", `http://localhost:8887/` + stateToQuerystring(state));
	request.send();
}
